...
function loadWidget(waifuPath, apiPath) {
	...
	function welcomeMessage() {
		...
		if (location.href.startsWith(SiteIndexUrl)) { //如果是主页
			...
			if (location.hostname == referrer.hostname) text = `欢迎来我家<span>『${document.title.split(" - ")[0]}』</span>`;
			else if (domain == "baidu") text = `Hello！来自 百度搜索 的朋友<br>你是搜索 <span>${referrer.search.split("&wd=")[1].split("&")[0]}</span> 找到的我吗？`;
			else if (domain == "so") text = `Hello！来自 360搜索 的朋友<br>你是搜索 <span>${referrer.search.split("&q=")[1].split("&")[0]}</span> 找到的我吗？`;
			else if (domain == "google") text = `Hello！来自 谷歌搜索 的朋友<br>欢迎阅读<span>『${document.title.split(" - ")[0]}』</span>`;
			else text = `Hello！来自 <span>${referrer.hostname}</span> 的朋友`;
		} else {
			text = `欢迎来我家<span>『${document.title.split(" - ")[0]}』</span>`;
		}
